...
static_assert(execution::is_bulk_twoway_executor_v<decltype(execution::require(inline_executor(),
        execution::blocking_adaptable, execution::twoway))>, "bulk two way executor requirements not met");
...
static_assert(!execution::is_twoway_executor_v<decltype(execution::require(inline_executor(),
        execution::blocking_adaptable, execution::twoway))>, "must not meet two way executor requirements");

int main()
{
  inline_executor ex1;
  auto ex2 = execution::require(ex1, execution::blocking_adaptable, execution::twoway);
