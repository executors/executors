...
struct eval<Executor, std::tuple<Args...>,
  typename type_check<decltype(
    ::std::experimental::concurrency_v2::execution::require(std::declval<Executor>(), std::declval<Args>()...)
  )>::type> : std::true_type {};

} // namespace can_require_impl

template<class Executor, class... Args>
struct can_require : can_require_impl::eval<Executor, std::tuple<Args...>> {};
