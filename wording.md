...
In the Table below, `x` denotes a (possibly const) executor object of type `X`, `fut` denotes a future object satisfying the Future requirements, `val` denotes any object stored in `fut`'s associated shared state when it becomes ready and nonexceptional, `e` denotes the object stored in `fut`'s associated shared state when it becomes ready and exceptional, `f` denotes a function object of type `F&&` callable as `DECAY_COPY(std::forward<F>(f))(val)` or `DECAY_COPY(std::forward<F>(f))(exception_arg, e)` and where `decay_t<F>` satisfies the `MoveConstructible` requirements, and `R` denotes the type of the expression `DECAY_COPY(std::forward<F>(f))(val)`.

| Expression | Return Type | Operational semantics |
|------------|-------------|---------------------- |
| `x.then_execute(f, fut)` | A type that satisfies the `Future` requirements for the value type `R`. | Let `NORMAL` be the expression `DECAY_COPY(std::forward<F>(f))(val)` and `EXCEPTIONAL` be the expression `DECAY_COPY(std::forward<F>(f))(exception_arg, e)`. <br/> <br/> When `fut` becomes ready, if it is nonexceptional, creates an execution agent which invokes `NORMAL` at most once, if `NORMAL` is a well-formed expression, with the call to `DECAY_COPY` being evaluated in the thread that called `then_execute`. <br/> <br/> Otherwise, if `e` exists, creates an execution agent which invokes `EXCEPTIONAL` at most once, if `EXCEPTIONAL` is a well-formed expression, with the call to `DECAY_COPY` being evaluated in the thread that called `then_execute`. <br/> <br/> If `NORMAL` and `EXCEPTIONAL` are both well-formed expressions, `decltype(EXCEPTIONAL)` shall be convertible to `R`. <br/> <br/> If neither `NORMAL` nor `EXCEPTIONAL` are well-formed expresions, the invocation of `then_execute` shall be ill-formed. <br/> <br/> May block pending completion of `NORMAL` or `EXCEPTIONAL`. <br/> <br/> The invocation of `then_execute` synchronizes with (C++Std [intro.multithread]) the invocation of `f`. <br/> <br/> Stores the result of either the `NORMAL` or `EXCEPTIONAL` expression, or any exception thrown by the expression, in the associated shared state of the resulting `Future`. Otherwise, if `e` exists, stores `e` in the associated shared state of the resulting `Future`. |
